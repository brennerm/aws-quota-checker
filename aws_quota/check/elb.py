...
def get_elbs(session: boto3.Session):
    return list(session.client("elbv2").describe_load_balancers()["LoadBalancers"])
...
class ListenerPerClassicLoadBalancerCountCheck(InstanceQuotaCheck):
    ...
    def get_all_identifiers(session: boto3.Session) -> typing.List[str]:
        return [
            lb["LoadBalancerName"]
            for lb in session.client("elb").describe_load_balancers()[
                "LoadBalancerDescriptions"
            ]
        ]
    ...
    def current(self):
        try:
            ...
            "elb"
        ).exceptions.AccessPointNotFoundException as e:
...
class ListenerPerNetworkLoadBalancerCountCheck(InstanceQuotaCheck):
    ...
    @staticmethod
    def get_all_identifiers(session: boto3.Session) -> typing.List[str]:
        return [nlb["LoadBalancerArn"] for nlb in get_nlbs(session)]
    ...
    def current(self):
        try:
            ...
            "elbv2"
        ).exceptions.LoadBalancerNotFoundException as e:
            raise InstanceWithIdentifierNotFound(self) from e
...
class TargetGroupCountCheck(QuotaCheck):
    ...
    def current(self):
        return len(
            self.boto_session.client("elbv2").describe_target_groups()["TargetGroups"]
        )
